...
// (2) 구독자 정보  모달 보기
function subscribeInfoModalOpen(pageId) {
	$.ajax({
		url:`/api/user/${pageId}/subscribe`,
		dataType: "json"
	}).done(res =>{
...
<div class="subscribe__btn">`;
	...
	if(!u.equalUserState){	//동일 유저가 아닐때 버튼이 생김
		if(u.subscribeState){ //이미 구독한 상태
...
function profileImageUpload(pageUserId, principalId) {
	...
	$("#userProfileImageInput").on("change", (e) => {
		...
		$.ajax({
			...
			data: formData,
			contentType: false, // x-www-form-urlencoded로 파싱되는것을 방지
			processData: false, // contentType을 false로 줬을 때 QueryString 자동 설정되는것을 방지
